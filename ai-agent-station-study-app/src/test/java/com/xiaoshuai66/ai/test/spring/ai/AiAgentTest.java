...
import com.xiaoshuai66.ai.test.spring.ai.advisors.RagAnswerAdvisor;
...
public class AiAgentTest {
    ...
    public void init() {
        ...
        OpenAiApi openAiApi = OpenAiApi.builder()
                ...
                .apiKey("sk-412acdee88d9445cb588171e32c242da")
...
//                .embeddingsPath("v1/embeddings")
    ...
    public void test_client03() {
        ChatClient chantClient01 = ChatClient.builder(chatModel)
                .defaultSystem("""
                        ...
                        """)
                .defaultAdvisors(
                        // 启动窗口式对话记忆，只保留最近100条消息参与上下文
                        PromptChatMemoryAdvisor.builder(
                                MessageWindowChatMemory.builder()
                                        .maxMessages(100)
                                        .build()
                        ).build(),
                        // 开启RAG，把向量库中"knowledge == 'article-prompt-words'"命名空间最相近的5段文档检索出来，拼接提示词用于回答
                        new RagAnswerAdvisor(vectorStore, SearchRequest.builder()
                                .topK(5)
                                .filterExpression("knowledge == 'article-prompt-words'")
                                .build())
...
//                .defaultTools(new SyncMcpToolCallbackProvider(sseMcpClient01(), sseMcpClient02()))
        ...
        System.out.println("\n>>> ASSISTANT: " + chatClient02
                .prompt(userInput)
                // 给系统提示词里的占位符 current_data 传值
                .system(s -> s.param("current_date", LocalDate.now().toString()))
                // 向已配置的顾问链传入运行参数
                .advisors(a -> a
                        // 设置会话id，用于把消息归入同一会话，检索历史时按该id聚合
                        .param(CHAT_MEMORY_CONVERSATION_ID_KEY, "chatId-101")
                        // 设置记忆中取回历史消息条数上线（这里为100）,影响对话记忆注入的长度
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 100))
                .call().content());
    }
    ...
    public McpSyncClient stdioMcpClient() {
        ...
        var stdioParams = ServerParameters.builder("npx")
                .args("-y", "@modelcontextprotocol/server-filesystem", "/Users/zhaoshuai/Desktop", "/Users/zhaoshuai/Desktop")
                .build();
    ...
    public McpSyncClient sseMcpClient01() {
        HttpClientSseClientTransport sseClientTransport = HttpClientSseClientTransport.builder("http://124.71.185.29:8102").build();

        McpSyncClient mcpSyncClient = McpClient.sync(sseClientTransport).requestTimeout(Duration.ofSeconds(180)).build();
    ...
    public McpSyncClient sseMcpClient02() {
        HttpClientSseClientTransport sseClientTransport = HttpClientSseClientTransport.builder("http://124.71.185.29:8101").build();
